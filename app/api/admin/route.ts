...
import { sendKYCStatusEmail, sendDepositConfirmEmail } from '@/lib/email';
        ...
        stats: {
          ...
          transactions: {
            deposits: {
              ...
              volume: transactions.filter(t => t.type === 'deposit' && t.status === 'completed').reduce((sum, t) => sum + t.amount, 0),
            },
            withdrawals: {
              ...
              volume: transactions.filter(t => t.type === 'withdrawal' && t.status === 'completed').reduce((sum, t) => sum + t.amount, 0),
            },
          },
        },
      });
      ...
      if (depositUser) {
        try {
          await sendDepositConfirmEmail(depositUser.email, {
            ...
            transactionId: transaction.oderId,
          });
        } catch (e) {
