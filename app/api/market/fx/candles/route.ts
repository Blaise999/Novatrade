...
export async function GET(req: Request) {
  try {
    ...
    const tfRaw = searchParams.get('tf') ?? '15m';
    const tf = normalizeTF(tfRaw);

    // allow tiny limits for debugging (you used limit=2)
    const limit = Math.min(Math.max(Number(searchParams.get('limit') ?? 120), 2), 500);
    ...
    return NextResponse.json(
      { ok: true, symbol, tf: tfRaw, candles },
