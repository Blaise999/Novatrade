...
export async function GET(req: Request) {
  try {
    ...
    // Finnhub items: { description, displaySymbol: "EUR/USD", symbol: "OANDA:EUR_USD" }
    const pairs = data.map((x: any) => ({
      id: String(x.symbol ?? x.displaySymbol),
      displaySymbol: String(x.displaySymbol ?? x.symbol), // "EUR/USD"
      symbol: String(x.symbol ?? x.displaySymbol),        // "OANDA:EUR_USD"
    ...
    return NextResponse.json(
      ...
      {
        headers: {
          // fast: cache the big list at the edge/CDN
