...
export default function StockTradingPage() {
  ...
  // When selected asset changes, reset price state (clean + correct)
  useEffect(() => {
    const p = selectedAsset.price;
    setCurrentPrice(p);
    setBidPrice(p * 0.9999);
    setAskPrice(p * 1.0001);
  }, [selectedAsset]);

  // Filter assets
  const filteredAssets = stockAssets.filter(
    (asset) =>
      asset.symbol.toLowerCase().includes(searchQuery.toLowerCase()) ||
      asset.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // Update prices effect (simulated ticker)
  useEffect(() => {
  ...
  }, [selectedAsset.price, currentPrice]);
  ...
  const effectiveShares = orderMode === 'shares' ? shareQty : Math.floor(dollarAmount / askPrice);
  ...
  const getMarketStatus = () => {
    ...
    if (hour >= 21 && hour < 24) return { status: 'After Hours', color: 'text-electric' };
  ...
  const info =
    stockInfo[selectedAsset.symbol] || ({ emoji: '📈', color: 'from-gray-500 to-gray-600', sector: 'Other' } as const);
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            <div className="text-center">
              ...
              {/* ✅ changePercent24h (matches your types) */}
              <p className={`text-xs ${selectedAsset.changePercent24h >= 0 ? 'text-profit' : 'text-loss'}`}>
            ...
            >
              <defs>
                <linearGradient id="stockGradient" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <stop
                    offset="0%"
                    stopColor={selectedAsset.changePercent24h >= 0 ? 'rgb(0, 217, 165)' : 'rgb(239, 68, 68)'}
                    stopOpacity="0.3"
                  />
                  <stop
                    offset="100%"
                    ...
                    <>
                      <path
                        d={`M ${points[0].x} ${points[0].y} ${points
                          ...
                          .join(' ')}`}
                        fill="none"
                        stroke={selectedAsset.changePercent24h >= 0 ? '#00d9a5' : '#ef4444'}
                        strokeWidth="2"
                      />
                      <path
                        d={`M ${points[0].x} ${points[0].y} ${points
                          .slice(1)
                          .map((p) => `L ${p.x} ${p.y}`)
                          .join(' ')} L ${points[points.length - 1].x} ${chartDimensions.height} L ${points[0].x} ${
                    ...
                    <div key={pos.id} className="p-3 bg-white/5 rounded-xl">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                        ...
                        <div>
                          ...
                          {/* ✅ avgEntry (matches your types) */}
                          <p className="text-cream font-mono">${pos.avgEntry.toFixed(2)}</p>
                        </div>
                        <div>
                ...
                <tbody>
                  {stockPositions.map((pos) => (
                    <tr key={pos.id} className="border-t border-white/5">
                      ...
                      <td className="py-2 text-right font-mono text-cream">${pos.avgEntry.toFixed(2)}</td>
                ...
                {favorites.length > 0 && (
                  <div className="mb-4">
                    ...
                    {filteredAssets
                      ...
                      .map((asset) => {
                        const assetInfo =
                          stockInfo[asset.symbol] || ({ emoji: '📈', color: 'from-gray-500 to-gray-600', sector: 'Other' } as const);
                        return (
                          <button
                            key={asset.symbol}
                            onClick={() => {
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="p-3 bg-white/5 rounded-xl space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-cream font-mono">${positionToSell.avgEntry.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between pt-2 border-t border-white/10">
                    ...
                    <span
                      className={`font-semibold ${
                        (bidPrice - positionToSell.avgEntry) * sellQty >= 0 ? 'text-profit' : 'text-loss'
                      }`}
                    >
                      {(bidPrice - positionToSell.avgEntry) * sellQty >= 0 ? '+' : ''}
                      ${((bidPrice - positionToSell.avgEntry) * sellQty).toFixed(2)}
                    </span>
                  </div>
                </div>
              </div>
