...
// ✅ Add this helper file: lib/zustandStorage.ts (the one you pasted)
// and import it here:
...
// ------------------------
// Date safety (persist turns Date -> string)
// ------------------------
...
function reviveSignal(sig: any): TradeSignal {
  return {
    ...sig,
    startTime: toDate(sig?.startTime),
    endTime: toDate(sig?.endTime),
    createdAt: toDate(sig?.createdAt),
  };
}
function reviveSession(s: any): TradingSession {
  return {
    ...s,
    startTime: toDate(s?.startTime),
    endTime: toDate(s?.endTime),
    createdAt: toDate(s?.createdAt),
    signals: Array.isArray(s?.signals) ? s.signals.map(reviveSignal) : [],
  };
}
function reviveAdmin(a: any): AdminUser {
  return {
    ...a,
    createdAt: toDate(a?.createdAt),
    lastLogin: a?.lastLogin ? toDate(a.lastLogin) : new Date(),
  };
}
...
export const useAdminAuthStore = create<AdminAuthStore>()(
  persist(
    ...
    }),
    {
      ...
      storage: safeSessionStorage(),      // ✅ sessionStorage + SSR-safe
      skipHydration: true,                // ✅ prevents #418/#423 mismatch
      partialize: (state) => ({
        admin: state.admin,
      ...
      merge: (persisted, current) => {
        ...
        const merged: any = { ...current, ...p };
        if (merged.admin) merged.admin = reviveAdmin(merged.admin);
        return merged;
      },
    }
  )
  ...
  persist(
    (set, get) => ({
        ...
        set((state) => ({
          sessions: state.sessions.map((s) =>
            s.id === sessionId
              ? {
                  ...s,
                  // ✅ Date-safe sort (works after persist rehydrate too)
                  signals: [...s.signals, signal].sort((a, b) => toMs(a.startTime) - toMs(b.startTime)),
                }
              : s
          ),
          activeSession:
        ...
        set((state) => ({
          sessions: state.sessions.map((s) =>
            s.id === sessionId
              ? {
                  ...s,
                  signals: s.signals.map((sig) => (sig.id === signalId ? { ...sig, ...updates } : sig)),
                }
              : s
          ),
          activeSession:
      ...
      removeSignal: (sessionId, signalId) => {
        set((state) => ({
          sessions: state.sessions.map((s) =>
            s.id === sessionId ? { ...s, signals: s.signals.filter((sig) => sig.id !== signalId) } : s
          ),
          activeSession:
            state.activeSession?.id === sessionId
      ...
      bulkAddSignals: (sessionId, signals) => {
        ...
        const newSignals: TradeSignal[] = signals.map((sig, index) => {
          const startMs = toMs(sig.startTime);
          return {
            ...
            endTime: new Date(startMs + 300000), // 5 min default
            duration: 300,
        ...
        set((state) => ({
          sessions: state.sessions.map((s) =>
            s.id === sessionId
              ? { ...s, signals: [...s.signals, ...newSignals].sort((a, b) => toMs(a.startTime) - toMs(b.startTime)) }
              : s
          ),
        }));
      },
      ...
      getCurrentSignal: (assetId, time = new Date()) => {
        ...
        const currentSignal = activeSession.signals.find((signal) => {
          const start = toMs(signal.startTime);
          const end = toMs(signal.endTime);
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        const p = (persisted as any) || {};
        const merged: any = { ...current, ...p };
        merged.sessions = Array.isArray(merged.sessions) ? merged.sessions.map(reviveSession) : [];
        // activeSession isn’t persisted by partialize; keep current default.
        return merged;
      },
    }
  )
...
/**
 * IMPORTANT:
 * Because skipHydration: true is set, you MUST rehydrate on client once, e.g. in app/providers.tsx:
 * useEffect(() => { useAdminAuthStore.persist.rehydrate(); useAdminSessionStore.persist.rehydrate(); }, []);
 */
